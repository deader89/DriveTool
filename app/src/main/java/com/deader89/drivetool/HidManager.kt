...
object HidManager {
    ...
    fun setupHidNodes(modulePath: String? = null): Result<Unit> {
        ...
        script.append("""
            ...
            # Falls bereits existiert, erst abbauen
            if [ -d $CONFIGFS_HOME ]; then
                echo "" > $CONFIGFS_HOME/UDC 2>/dev/null
                sleep 0.1
                rm -f $CONFIGFS_HOME/configs/c.1/hid.usb0 2>/dev/null
                rm -f $CONFIGFS_HOME/configs/c.1/hid.usb1 2>/dev/null
                rm -f $CONFIGFS_HOME/configs/c.1/mass_storage.0 2>/dev/null
                rmdir $CONFIGFS_HOME/configs/c.1/strings/0x409 2>/dev/null
                rmdir $CONFIGFS_HOME/configs/c.1 2>/dev/null
                rmdir $CONFIGFS_HOME/functions/hid.usb0 2>/dev/null
                rmdir $CONFIGFS_HOME/functions/hid.usb1 2>/dev/null
                rmdir $CONFIGFS_HOME/functions/mass_storage.0/lun.0 2>/dev/null
                rmdir $CONFIGFS_HOME/functions/mass_storage.0 2>/dev/null
                rmdir $CONFIGFS_HOME/strings/0x409 2>/dev/null
                cd / && rmdir $CONFIGFS_HOME 2>/dev/null
            fi
            ...
            mkdir -p $CONFIGFS_HOME 2>&1 || exit 1
            cd $CONFIGFS_HOME 2>&1 || exit 1
            
            echo 0x1d6b > idVendor 2>&1
            echo 0x0104 > idProduct 2>&1
            echo 0x0100 > bcdDevice 2>&1
            echo 0x0200 > bcdUSB 2>&1
            
            mkdir -p strings/0x409 2>&1
            echo "deadbeaf" > strings/0x409/serialnumber 2>&1
            echo "Deader89" > strings/0x409/manufacturer 2>&1
            echo "DriveTool HID" > strings/0x409/product 2>&1
            
            mkdir -p configs/c.1/strings/0x409 2>&1
            echo "Config 1: HID Only" > configs/c.1/strings/0x409/configuration 2>&1
            echo 250 > configs/c.1/bmAttributes 2>&1
            
            # Tastatur (hidg0)
            mkdir -p functions/hid.usb0 2>&1
            echo 1 > functions/hid.usb0/protocol 2>&1
            echo 1 > functions/hid.usb0/subclass 2>&1
            echo 8 > functions/hid.usb0/report_length 2>&1
            echo -ne \\x05\\x01\\x09\\x06\\xa1\\x01\\x05\\x07\\x19\\xe0\\x29\\xe7\\x15\\x00\\x25\\x01\\x75\\x01\\x95\\x08\\x81\\x02\\x95\\x01\\x75\\x08\\x81\\x03\\x95\\x05\\x75\\x01\\x05\\x08\\x19\\x01\\x29\\x05\\x91\\x02\\x95\\x01\\x75\\x03\\x91\\x03\\x95\\x06\\x75\\x08\\x15\\x00\\x25\\x65\\x05\\x07\\x19\\x00\\x29\\x65\\x81\\x00\\xc0 > functions/hid.usb0/report_desc 2>&1
            ln -s functions/hid.usb0 configs/c.1/ 2>&1
            
            # Maus (hidg1)
            mkdir -p functions/hid.usb1 2>&1
            echo 2 > functions/hid.usb1/protocol 2>&1
            echo 1 > functions/hid.usb1/subclass 2>&1
            echo 4 > functions/hid.usb1/report_length 2>&1
            echo -ne \\x05\\x01\\x09\\x02\\xa1\\x01\\x09\\x01\\xa1\\x00\\x05\\x09\\x19\\x01\\x29\\x03\\x15\\x00\\x25\\x01\\x75\\x01\\x95\\x03\\x81\\x02\\x95\\x01\\x75\\x05\\x81\\x03\\x05\\x01\\x09\\x30\\x09\\x31\\x09\\x38\\x15\\x81\\x25\\x7f\\x75\\x08\\x95\\x03\\x81\\x06\\xc0\\xc0 > functions/hid.usb1/report_desc 2>&1
            ln -s functions/hid.usb1 configs/c.1/ 2>&1
            ...
            if [ ! -z "${'$'}UDC_DEV" ]; then
                ...
                # Kontrolle ob Bindung erfolgreich
            ...
            # Berechtigungen
            chmod 666 $HID_KEYBOARD 2>&1
            chmod 666 $HID_MOUSE 2>&1
            magiskpolicy --live "allow untrusted_app uhid_device chr_file { getattr open read write ioctl }" 2>/dev/null
            chcon u:object_r:uhid_device:s0 $HID_KEYBOARD 2>&1
            chcon u:object_r:uhid_device:s0 $HID_MOUSE 2>&1
    ...
    fun setupHostingNodes(imagePath: String, asReadonly: Boolean = true): Result<Unit> {
        ...
        script.append("""
            ...
            # CD-ROM Mode aktivieren wenn readonly
    ...
    fun teardown(): Result<Unit> {
        ...
        val script = """
            ...
            # 1. UDC explizit lösen und Hardware trennen
            if [ -d ${'$'}CONFIGFS_HOME ]; then
                echo "" > ${'$'}CONFIGFS_HOME/UDC 2>/dev/null
                echo "" > ${'$'}CONFIGFS_HOME/functions/mass_storage.0/lun.0/file 2>/dev/null
                sleep 0.2
                
                # 2. Inside-Out Löschen der ConfigFS Struktur
                rm -f ${'$'}CONFIGFS_HOME/configs/c.1/hid.usb0 2>/dev/null
                rm -f ${'$'}CONFIGFS_HOME/configs/c.1/hid.usb1 2>/dev/null
                rm -f ${'$'}CONFIGFS_HOME/configs/c.1/mass_storage.0 2>/dev/null
                rmdir ${'$'}CONFIGFS_HOME/configs/c.1/strings/0x409 2>/dev/null
            ...
            # 3. Android USB-Stack aggressiv zur Neu-Initialisierung zwingen
            # Wir setzen zuerst auf 'none' und triggern dann MTP
            setprop sys.usb.config none
            setprop sys.usb.configfs 1
            start adbd 2>/dev/null
            sleep 0.5
            
            # MTP erzwingen (oft hilft mtp,adb um den Handshake zu triggern)
            setprop sys.usb.config mtp,adb
            
            # 4. Falls MTP noch nicht aktiv, UDC des Standard-Gadgets (g1) prüfen
            sleep 0.5
            if [ -e /config/usb_gadget/g1/UDC ] && [ -z "$(cat /config/usb_gadget/g1/UDC 2>/dev/null)" ]; then
                UDC_DEV=$(ls /sys/class/udc | head -n 1)
                echo "${'$'}UDC_DEV" > /config/usb_gadget/g1/UDC 2>/dev/null
            fi
        """.trimIndent()
